...
家で普通にインターネットを使う分には、インターネット回線は1回線あれば十分なんだけど、2022年7月にauで大規模通信障害があったように、インターネット回線もいつ何時障害が起きるか分からない。回線が死ぬともう手も足も出せず、通信キャリアの5Gに繋がったスマホで会社のSlackを開いて「おうちのインターネットが死んじゃった……」と書き込むしかなくなってしまうので、そうならないよう回線を多重化しておくことにした。

いまはJ:COMの1Gコースをメインの回線、そしてNTTのフレッツ光ネクスト マンションタイプをフェイルオーバー@<fn>{failover}用の回線として契約している。実際、何度かJ:COMで回線障害が起きたときに、フレッツ光へフェイルオーバーして事なきを得たことがある。
...
回線を多重化したので、「メインの回線が死んだら、自動でフェイルオーバーする」という機能を持ったルーターを購入。これで「J:COMの回線が障害？じゃあフレッツ光にフェイルオーバーするぞ！」と手動で頑張らなくても、勝手にフェイルオーバーされるようになった。（@<img>{brtac828}）
...
2022年3月、東京の電気使用量が供給量に達して停電になるかも、というニュースが出た。もともと大きめの地震があるたびに、首から下げるタイプの懐中電灯とか、電池のストックとかは小まめに買い足していたので、防災用品は大分揃っている。ポータブル電源だけは「今までより容量が大きくて値段が安いもの」が毎年出るので、いつ買うか迷ってたんだけど、セールで少し安くなってたのでついにAnker PowerHouse II 800を購入！
